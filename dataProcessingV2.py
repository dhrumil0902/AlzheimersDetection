...
ALPHA_FREQUENCIES = (4, 8, 0.05)
GAMMA_FREQUENCIES = (15, 35, 0.5)
CWT_B = 6
CWT_C = 0.8125
...
def calculate_pac(theta_phase, gamma_amplitude, nbins=18):
    ...
    for i in range(nbins):
        ...
        amplitude_means.append(np.mean(gamma_amplitude[indices]))
    amplitude_prob_dist = np.array(amplitude_means) / np.sum(amplitude_means)
        ...
        for epoch_id in range(N_EPOCHS):
            ...
            for segment_id in range(N_SEGMENTS):
                ...
                for channel_id in range(N_CHANNELS):
                    ...
                    filtered_segment[channel_id, :] = bandpass_filter(notch_filtered_signal, 4, 35, SAMPLING_FREQUENCY_HZ)
